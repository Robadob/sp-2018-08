...
__global__ void atomicHistogram(unsigned int* bin_index, unsigned int* bin_sub_index, unsigned int *pbm_counts, glm::vec4 *messageBuffer)
{
    ...
    glm::ivec3 gridPos = getGridPosition(glm::vec3(messageBuffer[index]));
...
#define CIRCLES
    ...
    glm::ivec3 tGridPos;
    glm::ivec3 topLeft = getGridPosition(pos - d_RADIUS);
    glm::ivec3 bottomRight = getGridPosition(pos + d_RADIUS);
    unsigned int count = 0;
    glm::vec3 average = glm::vec3(0);
    for (tGridPos.z = topLeft.z; tGridPos.z <= bottomRight.z; tGridPos.z++)
    {
...
#else
            unsigned int binHash = getHash(glm::ivec3(topLeft.x, tGridPos.y, tGridPos.z));
            unsigned int binStart = tex1Dfetch(d_texPBM, binHash);
            binHash = getHash(glm::ivec3(bottomRight.x, tGridPos.y, tGridPos.z));
            unsigned int binEnd = tex1Dfetch(d_texPBM, binHash + 1);
#endif
            if (binHash>d_gridDim*d_gridDim*d_gridDim)
            {
...
#ifndef CIRCLES
out[index] = glm::vec4(average, 0);
#else
out[index] = glm::vec4(pos + average,0);
#endif
}
...
void run(std::ofstream &f, const unsigned int ENV_WIDTH, const unsigned int AGENT_COUNT = 1000000)
{
    ...
    //const unsigned int ENV_WIDTH = 40; 
    float ENV_WIDTH_float = (float)ENV_WIDTH;
    const unsigned int RNG_SEED = 12;
    //const unsigned int AGENT_COUNT = 1000000;
    ...
    glm::vec4 *h_out0 = (glm::vec4*)malloc(sizeof(glm::vec4) * AGENT_COUNT);
    //Init agents
    {
        //Generate curand
        curandState *d_rng;
    ...
    const float RADIAL_VOLUME = (4.0f/3.0f) * glm::pi<float>()*RADIUS*RADIUS*RADIUS;
    const unsigned int AVERAGE_NEIGHBOURS = (unsigned int)(AGENT_COUNT*RADIAL_VOLUME / ENV_VOLUME);
    printf("Average Neighbours: %d\n", AVERAGE_NEIGHBOURS);
    ...
    for (unsigned int k = 0; k < 2; ++k)
    {
        ...
        const glm::uvec3 GRID_DIMS = glm::uvec3((unsigned int)ceil(GRID_DIMS_float));
        ...
        CUDA_CALL(cudaMemcpy(k==0 ? h_out : h_out0, d_out, sizeof(glm::vec4)*AGENT_COUNT, cudaMemcpyDeviceToHost));
        if (k!=0)
        {
            for (unsigned int i = 0; i < AGENT_COUNT; ++i)
            {
                ...
                {
                    if (fails == 0)
                        printf("(%.5f, %.5f, %.5f) vs (%.5f, %.5f, %.5f)\n", h_out[i].x, h_out[i].y, h_out[i].z, h_out0[i].x, h_out0[i].y, h_out0[i].z);
                    fails++;
                }
            }
            if (fails > 0)
...
{
    //density refers to number of neighbours
...
{
    {
        ...
        {
            //Run i agents in a density with roughly 100 radial neighbours, and log
