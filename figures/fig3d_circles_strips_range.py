...
###
csvDefault = loadCSV('[3D] Default, Circles Model.csv');
csvStrips = loadCSV('[3D] Strips, Circles Model.csv');
...
###
desiredWidths = [1.0, 0.4, 0.5, 0.7, 0.666667];
minWidth = -1;
colorI = 0;
while True:
    ...
    for i in range(len(binWidth[0])):
        if binWidth[0][i]>minWidth and binWidth[0][i]<t_minWidth and (binWidth[0][i] in desiredWidths):
    ...
    for j in range(len(binWidth)):
        ...
        for i in range(len(binWidth[j])):
            if binWidth[j][i]==minWidth:## and radialNeighbour[j][i] < 200:# and fails[j][i]==0:# #Toggle full vs low density
        ...
        #Send to plot
        if(len(x)>2):
            plotLine('%s %.2f' % (models[j], minWidth), x, y, colors[colorI], '-');
            colorI+=1;
...
### Export/Show Plot
###
plt.savefig('[3D] Circles Strips Range.eps')
plt.savefig('[3D] Circles Strips Range.pdf')
plt.savefig('[3D] Circles Strips Range.png')
plt.close();
#plt.show();
